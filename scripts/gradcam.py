...
class GradCam:
    ...
    def _get_features_hook(self, module, input, output):
        self.feature = self.reshape_transform(output[0].permute(1,0,2))  # Store and reshape the output features

    # Hook to get gradients from the backward pass
    def _get_grads_hook(self, module, input_grad, output_grad):
        self.gradient = self.reshape_transform(output_grad[0].permute(1,0,2))  # Store and reshape the output gradients
    ...
    def reshape_transform(self, tensor, height=24, width=8):
    ...
    def __call__(self, inputs):
        ...
        output, _ = self.model(inputs)  # Forward pass
        output = 

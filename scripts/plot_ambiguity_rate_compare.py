...
def compute_margins_for_checkpoint(
    ...
    try:
        ...
        rows, skipped, scale_metadata = compute_margin_rows_with_scale(
            query_pids,
            gallery_pids,
            lambda start, end: sim[start:end],
            margin_scale=margin_scale,
            scale_eta=scale_eta,
...
def main() -> None:
    ...
    if args.skip_test_eval:
        baseline_model_args.selected_inference = baseline_meta.get("inference")
        ours_model_args.selected_inference = ours_meta.get("inference")

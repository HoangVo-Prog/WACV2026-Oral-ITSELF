...
class PrototypeBranch(nn.Module):
    ...
    def forward(self, image_features, text_features, pids):
        image_features, text_features = self._project(image_features, text_features)
        zero = image_features.sum() * 0.0
        if not self.is_ready():
            return {"proto_id_loss": zero, "proto_rank_loss": zero}

        pids = pids.long()
        proto_id_loss = symmetric_identity_proxy_loss(
            image_features,
            text_features,
            pids,
            self.memory,
        ...
        proto_rank_loss = prototype_pair_ranking_loss(
            proto_scores,
            pids,
        ...
        return {"proto_id_loss": proto_id_loss, "proto_rank_loss": proto_rank_loss}

...
class PrototypeMemory(nn.Module):
    ...
    @torch.no_grad()
    def refresh(self, image_features, text_features, pids, num_iters=20,
                kmeans_init="deterministic", seed=None, alpha=0.35,
                refresh_epoch=None, refresh_step=-1):
        ...
        self.image_prototypes.copy_(self._blend_bank(old_image, image_bank, alpha))
        self.text_prototypes.copy_(self._blend_bank(old_text, text_bank, alpha))
        ...
        self.text_to_image.copy_(self._blend_bank(old_text_to_image, text_to_image, alpha))
        self.image_to_text.copy_(self._blend_bank(old_image_to_text, image_to_text, alpha))
    ...
    def _build_pbt_banks(self, image_features, text_features, pids, image_bank, text_bank):
        ...
        image_bank = F.normalize(image_bank.to(self.image_prototypes.device).float(), p=2, dim=1)
        text_bank = F.normalize(text_bank.to(self.text_prototypes.device).float(), p=2, dim=1)

...
def _host_aligned_gates(image_host_features, text_host_features, pids, epoch=None, warmup_epochs=0, eps=1e-6):
    ...
    if batch_size < 2 or image_host_features is None or text_host_features is None:
    ...
    alpha = torch.tensor(_HAPG_ALPHA_MAX * progress, device=device, dtype=torch.float32)
...
def identity_proxy_contrastive_per_sample(features, pids, prototypes, proto_pids, tau=0.05, hard_k=16):
    ...
    if features.numel() == 0 or prototypes.numel() == 0:
        return losses, {"valid_mask": valid, "hard_k": features.new_tensor(float(_positive_int(hard_k, 16)))}
...
def symmetric_identity_proxy_loss(
    ...
    if pressure_mode == "host_aligned":
        ...
        _, _, gate_details = _host_aligned_gates(None, None, pids)
        image_loss = image_losses[image_valid].mean() if image_valid.any() else image_losses.new_zeros(())
        text_loss = text_losses[text_valid].mean() if text_valid.any() else text_losses.new_zeros(())

    loss = 0.5 * (image_loss + text_loss)
    if not return_details:
        return loss
    details = {
        "prototype_k_img": image_features.new_tensor(float(hard_k)),
        "prototype_k_txt": text_features.new_tensor(float(hard_k)),
    }
